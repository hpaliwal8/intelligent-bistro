...
1. **Browse** — Menu tab → tap a card → modifier sheet → "Add to cart"
2. **Cart** — Cart tab → adjust quantity, remove an item, see totals update
3. **AI add** — Assistant tab → *"Add a Smash Burger with bacon and an iced latte"* — two tool calls in one turn
4. **AI edit** — *"Actually make it two burgers"* — `update_quantity` on the existing line
5. **AI guard-rail** — *"Do you have pizza?"* — graceful decline + Margherita Flatbread suggestion (no fabrication)
6. **AI clarification** — *"I want a sandwich"* — assistant asks the required spice level before adding
